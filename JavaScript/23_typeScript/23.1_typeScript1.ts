...
function greetUser1(user: object){
    console.log(`greetUser1 : hello ${user.name}`); //name 속성이 object 자료형에는 정의되어 있지 않음
}
greetUser1({name : 'Shawn', age : 28});

function greetUser2(user: {name: string, age: number}){ //해당 객체의 속성을 더 자세히 정의할것
    console.log(`greetUser2 : hello ${user.name}`);
}
...
enum Status1 {deleted, pending, active};
const blogPostStatus1: Status1 = Status1.active;
console.log(blogPostStatus1);    //2

enum Status2{deleted = -2, pending, active};
const blogPostStatus2: Status2 = Status2.active;
